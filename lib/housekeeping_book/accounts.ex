defmodule HousekeepingBook.Accounts do
  ...
  alias HousekeepingBook.Accounts.{Users, UserTokens, UserNotifier}
  ...
  def get_or_create_user_by_type(type) do
    case Repo.get_by(User, type: type) do
      nil -> register_user(%{type: type, name: "share", email: nil, password: "123412341234"})
      user -> {:ok, user}
    end
  end
  ...
  def get_or_create_user_by_name(name) do
    case Repo.get_by(User, name: name) do
      nil -> register_user(%{type: :normal, name: name, email: nil, password: "123412341234"})
      user -> {:ok, user}
    end
  end
  ...
  def change_user_email(user, attrs \\ %{}) do
    Users.email_changeset(user, attrs, validate_email: false)
  end
  ...
  defp user_email_multi(user, email, context) do
    ...
    |> Ecto.Multi.delete_all(:tokens, UserTokens.by_user_and_contexts_query(user, [context]))
  end
  ...
  def update_user_password(user, password, attrs) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.update(:user, changeset)
    |> Ecto.Multi.delete_all(:tokens, UserTokens.by_user_and_contexts_query(user, :all))
    |> Repo.transaction()
    |> case do
  ...
  def delete_user_session_token(token) do
    Repo.delete_all(UserTokens.by_token_and_context_query(token, "session"))
    :ok
  end
  ...
  defp confirm_user_multi(user) do
    ...
    |> Ecto.Multi.delete_all(:tokens, UserTokens.by_user_and_contexts_query(user, ["confirm"]))
  end
